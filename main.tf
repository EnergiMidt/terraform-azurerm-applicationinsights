...
resource "azurerm_monitor_action_group" "action_group" {
  ...
  dynamic "arm_role_receiver" {
    ...
    content {
      name                    = arm_role_receiver.value[arm_role_receiver.key].value["name"]
      role_id                 = split("/", arm_role_receiver.value[arm_role_receiver.key].value["id"])[4] # https://github.com/hashicorp/terraform-provider-azurerm/issues/8553
